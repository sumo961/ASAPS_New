...
/**
 * Counters already drawn as placed elements on the current beat.
 *
 * A counter can appear twice on one screen: once in the character's HUD frame
 * and again as a `kind: 'meter'` element the author positioned on this beat.
 * Both are legitimate on their own, and together they are simply the same
 * number twice.
 *
 * The placed element wins, because it is the beat-specific decision — the HUD
 * is standing scenery, this is deliberate emphasis here. Only that counter is
 * suppressed; the character's other meters stay in the frame.
 *
 * Keys are `characterId::counterName`, so two characters may each keep their
 * own `trust`.
 */
export type PlacedMeterLocation = { kind?: string; characterId?: string; counterName?: string };

export function countersPlacedOnBeat(
  // Beats carry `locations` as an array when serialised and as a Map once the
  // engine has hydrated them; both reach this function.
  locations: PlacedMeterLocation[] | Map<string, PlacedMeterLocation> | null | undefined,
): Set<string> {
  const out = new Set<string>();
  const list: PlacedMeterLocation[] =
    locations instanceof Map ? Array.from(locations.values()) : Array.isArray(locations) ? locations : [];
  for (const loc of list) {
    if (loc?.kind === 'meter' && loc.counterName) {
      out.add(`${loc.characterId || ''}::${loc.counterName}`);
    }
  }
  return out;
}

/** True when this counter is already drawn as a placed element on the beat. */
export function isCounterPlaced(
  placed: Set<string> | null | undefined,
  characterId: string | undefined,
  counterName: string | undefined,
): boolean {
  if (!placed || placed.size === 0 || !counterName) return false;
  return placed.has(`${characterId || ''}::${counterName}`);
}
