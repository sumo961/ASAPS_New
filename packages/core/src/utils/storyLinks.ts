...
export function beatLinks(beat: any): StoryLink[] {
  ...
  // aiCondition: one link per category (the AI picks one) plus the fallback;
  // arBeat's fallback shares the parameter name.
  if (Array.isArray(p.categories)) {
    p.categories.forEach((c: any, i: number) => {
      if (c) push(out, id, c.targetId ?? c.target, 'category', c.name, false, `categories[${i}].${c.targetId !== undefined ? 'targetId' : 'target'}`);
    });
  }
  push(out, id, p.fallbackTarget, 'param-target', 'Fallback', false, 'fallbackTarget');

  // aiDialogTree: each exit target's id IS the beat it leaves to.
  if (Array.isArray(p.exitTargets)) {
    p.exitTargets.forEach((e: any, i: number) => {
      if (!e) return;
      const key = e.id !== undefined ? 'id' : e.targetId !== undefined ? 'targetId' : 'target';
      push(out, id, e[key], 'exit', typeof e.description === 'string' ? e.description.slice(0, 40) : undefined, false, `exitTargets[${i}].${key}`);
    });
  }

  // arBeat anchors (asaps:// URIs are jumps, not links — ARBeat.getConnections).
  if (Array.isArray(p.anchors)) {
    p.anchors.forEach((a: any, i: number) => {
      if (a && typeof a.onTap === 'string' && !a.onTap.startsWith('asaps://')) {
        push(out, id, a.onTap, 'anchor', a.label || a.id, false, `anchors[${i}].onTap`);
      }
    });
  }

  // gpsLocation / indoorLocation: each place leads somewhere.
  if (Array.isArray(p.xrLocations)) {
    p.xrLocations.forEach((l: any, i: number) => {
      if (l) push(out, id, l.target, 'xr-location', l.name || l.id, false, `xrLocations[${i}].target`);
    });
  }

  // aiSummary's Restart destination.
  push(out, id, p.restartTarget, 'restart', 'Restart', false, 'restartTarget');

  // State requirements: when unmet, the beat escapes to its fallback. The
  // field is beat-level (some AI output nests it in parameters).
  for (const req of [...(Array.isArray(beat.requires) ? beat.requires : []), ...(Array.isArray(p.requires) ? p.requires : [])]) {
    if (req) push(out, id, req.fallbackTarget, 'requires', 'requires');
  }
