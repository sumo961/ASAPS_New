...
import type { TranslationResource, TranslationManifest } from '../translation/types';
...
export interface SerializeInput {
  ...
  /** Translation resources to include in the project */
...
export interface DeserializeResult {
  ...
  manifest: DirectoryAssetManifest;
  storyMetadata: any;
  /** Translation resources loaded from translations/ directory */
  translations: TranslationResource[];
...
export function serializeToDirectory(input: SerializeInput): SerializeResult {
  ...
  if (input.translations && input.translations.length > 0) {
    // Write individual translation resource files
    for (const resource of input.translations) {
      files.push({
        path: `translations/${resource.languageCode}.strings.json`,
        content: deterministicStringify(resource),
      });
    }

    // Write translation manifest
    if (input.translationManifest) {
      files.push({
        path: 'translations/_manifest.json',
  ...
  // 10. VCS helper files
  // We deliberately do NOT route assets through Git LFS. The earlier
  // `assets/**/* filter=lfs` directive caused asset binaries to push as LFS
  // pointers when authors had git-lfs installed, then fail to come back on
  // clone (the in-app cloner can't run the LFS smudge filter against repos
  // whose owner hasn't enabled LFS storage). For the typical ASAPS project
  // size, plain blobs work fine and the workflow stays self-contained.
  files.push({
    path: '.gitattributes',
...
export async function deserializeFromDirectory(
  ...
  let translationManifest: TranslationManifest | undefined;
  const translationsPath = join(rootPath, 'translations');
  if (await reader.exists(translationsPath)) {
    // Read manifest
    const translationManifestPath = join(translationsPath, '_manifest.json');
    if (await reader.exists(translationManifestPath)) {
      translationManifest = JSON.parse(await reader.readText(translationManifestPath));
    }

    // Read all .strings.json files
    const translationFiles = await reader.listDir(translationsPath);
    for (const file of translationFiles) {
      if (!file.isDirectory && file.name.endsWith('.strings.json')) {
        const resourceJson = await reader.readText(join(translationsPath, file.name));
        translations.push(JSON.parse(resourceJson) as TranslationResource);
      }
    }

    if (translations.length > 0) {
      console.log('[DirectoryFormat] Loaded', translations.length, 'translation(s):', translations.map(t => t.languageCode));
    }
  }
