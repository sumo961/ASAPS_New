...
describe('dialogTree layout: one button column', () => {
  it('every button uses the authored column (x, width)', () => {
    ...
    // The first authored button sets the column for every button.
    const buttons = layout.toLocations().filter((l) => l.kind === 'button');
    expect(buttons.map((b) => [b.x, b.width])).toEqual([[295, 456], [295, 456], [295, 456]]);
  });
});
