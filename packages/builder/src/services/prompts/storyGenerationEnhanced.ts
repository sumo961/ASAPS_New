...
- **Fictional Time** (type: "fictionalTime"): Set or advance in-story date/time
  ...
  - ⚠️ CRITICAL — CUMULATIVE CONSISTENCY: the running clock = the initial "set" + the SUM of every "advance" along the path the player actually takes. Whenever a later beat's prose names a month or season ("December", "that autumn", "by spring"), the accumulated advances reaching that beat MUST land on/near that month. Do NOT under-author advances: if the story spans Jan→Dec, the advance amounts must total ~11 months, not stop in August while the text says December. After placing your advance beats, mentally sum them from the set date and verify the final beats' implied date matches their narration. A clock frozen months behind the prose is a generation defect.
  - ⚠️ CRITICAL — displayFormat MUST match advance granularity. The HUD displayFormat — at globalSettings.hudOverlays.fictionalTime.displayFormat, e.g. "globalSettings": { "hudOverlays": { "fictionalTime": { "enabled": true, "displayFormat": "date" } } } — is one of: 'date' (e.g. "14 August 2024"), 'datetime-12h'/'datetime-24h', 'time-12h'/'time-24h', 'day-number', 'year'. If you only ever advance by days/weeks/months/years (never minutes/hours), you MUST choose a date-only format ('date' or 'day-number' or 'year') — a datetime/time format would show a frozen, arbitrary clock time (e.g. always "9:04 PM"), which is worse than showing no time. Only choose a time-bearing format if the story actually advances minutes/hours. Equivalently: if the displayFormat shows a time, you MUST author advances that move the time-of-day.
  - ⚠️ CRITICAL — PROSE TIME ANCHORS MUST DRIVE THE CLOCK. Every concrete time reference you write into beat prose must be backed by a fictionalTime beat so the displayed clock agrees with the text. The displayed time must NEVER contradict what a beat says. Two cases: (a) ABSOLUTE anchor — the prose states a specific time/date ("It is 2 a.m.", "March 3rd", "Sunday morning") → emit a fictionalTime operation:"set" on or just before that beat with those exact components. (b) RELATIVE jump — the prose implies elapsed time ("an hour later", "the next morning", "by the time she got there it was nearly 3") → emit a fictionalTime operation:"advance" of the matching amount/unit before that beat. If a beat says "at 3 she ends up at the station", the clock at that beat must read 3:00 (or close), not whatever it was. Sweep the narration for clock/calendar phrases and ensure each is matched by a set/advance; an unmatched time phrase is a generation defect (it makes the HUD lie). This includes time-of-day movement when you use a time-bearing displayFormat — the prose's "2 a.m. → dawn" must be real hour advances, not a static 9:04 PM.
...
### Pattern: Convergent beats with branch-divergent wording (memory token)
...
The switch KEEPS the feelings the player built and applies the variant's shift on top: mood moves by (variant initialMood − base initialMood), each sentiment the variant lists moves by (its strength − the base's, 0 if the base had none), and sentiments it does not list are untouched. So a relationship arc survives the transition — author the variant's seeds as the change you want, not as a reset.
