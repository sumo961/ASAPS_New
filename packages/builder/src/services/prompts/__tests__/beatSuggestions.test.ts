...
describe('guidance stays current', () => {
  const prompt = buildBeatSuggestionsSystemPrompt({});

  it('names multiChoice as the default for branching', () => {
    // aa22d6ca made multiChoice the default in the story prompt; this file
    // never got the update and kept recommending dialogTree/movementChoice.
    expect(prompt).toMatch(/multiChoice.*DEFAULT for branching/);
  });

  it('lists multiChoice among the choice-bearing beats', () => {
    expect(prompt).toMatch(/Choice beats \(multiChoice, dialogTree, movementChoice, pickProp\)/);
  });

  it('includes weeks in the fictional-time units', () => {
    // 'weeks' was added to advanceFictionalTime; omitting it here suggests a
    // unit set narrower than the runtime accepts.
    expect(prompt).toMatch(/minutes, hours, days, weeks, months, years/);
  });

  it('knows consequences beyond the counter shorthand', () => {
    expect(prompt).toContain('addSentiment');
    expect(prompt).toContain('playSound');
  });

  it('warns against suggesting a write to a bound counter', () => {
    expect(prompt).toMatch(/Never suggest a counter effect targeting one/);
  });

  it('still derives the beat catalogue from the live schema', () => {
    // The catalogue is generated by buildBeatTypeDigest, so it cannot drift.
    // Guard that it is still wired up rather than hand-listed.
    expect(prompt).toContain('## Available Beat Types');
    expect(prompt).toMatch(/- explanation \("Explanation"/);
  });
});
