...
# Fix StoryPreview.tsx TypeScript type resolution issue
# Adds explicit type imports and assertions

FILE="/Users/hartmut/Library/Mobile Documents/com~apple~CloudDocs/Coding/Project Phoenix/asaps-modern/packages/builder/src/components/preview/StoryPreview.tsx"

echo "Fixing StoryPreview.tsx type resolution..."
...
export const StoryPreview: React.FC<StoryPreviewProps> = ({ story, settings, onClose }) => {
  ...
  // Use refs with explicit IRenderer typing
  ...
  useEffect(() => {
    ...
    // CRITICAL FIX: Always ensure we have a valid renderer
    // In Strict Mode, the renderer might exist but its root might be invalid
    if (!rendererRef.current) {
      console.log('[StoryPreview] Creating new renderer and engine');
      
      // Create renderer with the preview container
      const reactRenderer: IRenderer = new ReactRenderer({
        container: containerRef.current,
        width: 1024,
        height: 768,
      });

      // Create story engine with explicitly typed renderer
      ...
      // Ensure the renderer has a valid root for the current container
      // This handles Strict Mode remounts where the root might be lost
    ...
    return () => {
      ...
      // In Strict Mode, don't destroy the renderer - just stop the engine
      // This allows the renderer to be reused on remount
    };
  ...
  const startPreview = useCallback(async () => {
    ...
    try {
      ...
      // Handle timer expiration - navigate to timer target
      timerManager.on('timerExpired', async ({ name, targetBeat }) => {
        if (targetBeat && rendererRef.current) {
          ...
          if (beat) {
            // Stop current story execution
            if (engineRef.current) {
              engineRef.current.stop();
            }
            // Mark beat as visited
            context.markBeatVisited(targetBeat);
            // Execute the timer target beat with explicit type assertion
            try {
              const nextBeatId = await beat.execute(context, rendererRef.current!);
              if (nextBeatId) {
                // Continue story from next beat
                const nextBeat = story.getBeat(nextBeatId);
                if (nextBeat) {
                  context.markBeatVisited(nextBeatId);
                  await nextBeat.execute(context, rendererRef.current!);
                }
              }
            } catch (error) {
    ...
    <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-6xl h-5/6 flex flex-col">
        ...
        <div className="flex-1 flex overflow-hidden">
          {/* Preview Area - Full ReactRenderer Output */}
          <div className="flex-1 bg-gray-50">
            <div 
              ref={containerRef}
              className="h-full w-full bg-white overflow-auto"
            >
              {/* ReactRenderer will inject content here */}
              {!currentBeat && !isRunning && (
...
echo "✓ StoryPreview.tsx fixed with explicit type imports and assertions"
echo ""
echo "Key changes:"
echo "  1. Added explicit import: import type { IRenderer } from '@asaps/core';"
echo "  2. Typed ref as: useRef<IRenderer | null>(null)"
echo "  3. Used type assertion: const reactRenderer: IRenderer = new ReactRenderer(...)"
echo "  4. Added non-null assertions where needed: rendererRef.current!"
echo ""
