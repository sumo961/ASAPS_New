 ...
 */
export function buildAffectPromptSection(
 ...
 */
function autoFixOrphanBookmarkReferences(generated: any): void {
  if (!generated?.beats || !Array.isArray(generated.beats)) return;
  ...
  const collectFromEffects = (effects: any) => {
  ...
  const collectFromChoices = (choices: any) => {
    if (!Array.isArray(choices)) return;
    for (const ch of choices) {
      collectFromEffects(ch?.effects);
      if (ch?.dialogNode) collectFromChoices(ch.dialogNode.choices);
    }
  };
  for (const beat of generated.beats) {
    const params: any = beat.parameters || {};
    collectFromChoices(params.choices);
    if (params.dialogTree) collectFromChoices(params.dialogTree.choices);
    if (beat.type === 'updateAffect') collectFromEffects(params.effects);
  }
  ...
  for (const beat of generated.beats) {
    ...
    const cond: any = beat.parameters?.condition;
    if (!cond) continue;
    ...
    fixCount++;
    console.error(
      `[aiHelper.autoFix] ✓ Orphan bookmark "${refName}" on conditionBeat ${beat.id} ` +
  ...
  if (fixCount > 0) {
    console.error(
      `[aiHelper.autoFix] Auto-converted ${fixCount} orphan bookmark reference(s) to baseline:'initial'`
    );
  }
}
