...
export const BEAT_TYPES = {
    ...
    infoText: 'Narrative text with Continue button. SINGLE CONNECTION ONLY: can only connect to ONE target! ❌ WRONG: infoText with 2+ connections. ✓ For branching, use multiChoice (or dialogTree for multi-turn dialogue, movementChoice for spatial hotspots). Optional: textVariations (array) for random text selection at runtime.',
    ...
    multiChoice: '🚨 DEFAULT CHOICE BEAT for single-screen branching — use this for the common "ask a question, player picks one answer" pattern (scene decisions, simple NPC one-liners, action picks, story branches with no follow-up turn). Simpler than dialogTree (no nested follow-ups), more discoverable than movementChoice (no spatial layer). Parameters: question (the prompt text), choices (array of { id, text, target } with optional effects/conditions/soundEffect), optional speaker (NPC name; leave empty for narrator), choiceDelay (seconds before choices fade in), markVisited. Per-choice effects[] support the full surface (variable / counter / inventory / affect / etc.). MULTIPLE TARGETS: define targets in choices[].target — NOT in connections array. Example: { "question": "You stand at a crossroads.", "choices": [{ "id": "c1", "text": "Take the forest path", "target": "beat_forest" }, { "id": "c2", "text": "Follow the river", "target": "beat_river" }] }. Use dialogTree instead when the dialog needs MULTIPLE TURNS or NESTED choices.',
    dialogTree: 'Multi-turn NPC dialogue with NESTED choices — use this when the player needs a real back-and-forth (player asks → NPC responds → player picks a follow-up → NPC responds again). The nesting is the value. For simple one-shot choice screens, use multiChoice instead. MULTIPLE TARGETS: define targets in dialogTree.choices[].target parameter, NOT in connections array. Supports: choiceDelay (seconds), presentationMode ("positioned"/"chat-scroll"/"chat-bubble"), showAvatars (boolean), responseDelay (seconds for NPC typing indicator), markVisited (boolean for per-choice visited tracking). Choices can modify counters via counter/counterOperation/counterValue properties. RECURSIVE DIALOGS: use target "__self__" to loop back to the same dialogTree (e.g., interrogation, shopping). NPC AUTO-EXIT: A dialog node can have a "target" field to auto-advance WITHOUT showing choices (NPC dismissals, forced exits). When "target" is set on a node, choices are ignored. Example multi-turn: { "dialogTree": { "id": "root", "speaker": "Guard", "text": "Halt! Who goes there?", "choices": [{ "id": "c1", "text": "I am a traveler", "dialogNode": { "id": "n1", "speaker": "Guard", "text": "What is your business?", "choices": [{ "id": "c2", "text": "Just passing through", "target": "beat_pass" }] } }] } }.',
    movementChoice: '🚨 SPECIALIZED — NOT the default multi-choice beat! Renders choices as INVISIBLE HOTSPOTS on a background scene. Only use when the scene has a background image AND each choice maps to a spatial location on that image (e.g., clicking the library door, staircase, garden gate). For abstract choices (actions, answers, decisions), use multiChoice instead. When in doubt: use multiChoice for one-shot prompts, dialogTree for multi-turn dialogue. MULTIPLE TARGETS: define targets in choices[].target parameter, NOT in connections array. Each choice needs { id, text, location, target } - always set "location" to same value as "text". Supports: choiceDelay (seconds), markVisited (boolean), showTextOnHover (boolean).',
...
// Run `node scripts/check-affect-prompt-sync.js` (TODO) or diff manually
// against packages/core/src/prompts/affectPrompt.ts to verify drift is
// intentional.
// =====================================================================
/**
 ...
 */
export function buildAffectPromptSection(depth = 'auto') {
    const parts = [];
...
function buildDepthDialGuidance(depth) {
 ...
 */
function autoFixAiSummaryMaxLength(generated) {
    if (!generated?.beats || !Array.isArray(generated.beats))
        return;
    let fixCount = 0;
    for (const beat of generated.beats) {
        if (beat.type !== 'aiSummary')
            continue;
        if (!beat.parameters)
            continue;
...
/**
 * v0.9.46+ — auto-fix orphan bookmark references. Detects conditionBeats
 * with `baseline: { bookmark: "X" }` that have no upstream
 * `bookmarkAffectState` Effect taking that name, and converts those
 * references to `baseline: 'initial'` so the runtime reads against
 * story-start instead of silently resolving to zero. Preserves the
 * intent (delta-from-snapshot) while sidestepping the runtime-zero bug.
 *
 * Mirror of `autoFixOrphanBookmarkReferences` in
 * packages/builder/src/services/AIService.ts. Keep in sync.
 */
function autoFixOrphanBookmarkReferences(generated) {
    if (!generated?.beats || !Array.isArray(generated.beats))
        return;
    // Step 1: collect every bookmarkAffectState Effect's name, anywhere
    // it appears in the story.
    const takenBookmarks = new Set();
    const collectFromEffects = (effects) => {
    ...
    const collectFromChoices = (choices) => {
        if (!Array.isArray(choices))
            return;
        for (const ch of choices) {
            collectFromEffects(ch?.effects);
            if (ch?.dialogNode)
                collectFromChoices(ch.dialogNode.choices);
        }
    };
    for (const beat of generated.beats) {
        const params = beat.parameters || {};
        collectFromChoices(params.choices);
        if (params.dialogTree)
            collectFromChoices(params.dialogTree.choices);
        if (beat.type === 'updateAffect')
            collectFromEffects(params.effects);
    }
    // Step 2: walk conditionBeats, convert orphan refs to baseline:'initial'.
    let fixCount = 0;
    for (const beat of generated.beats) {
        if (beat.type !== 'conditionBeat')
            continue;
        const cond = beat.parameters?.condition;
        if (!cond)
            continue;
        const baseline = cond.baseline;
        if (!baseline || typeof baseline !== 'object' || !baseline.bookmark)
            continue;
        const refName = String(baseline.bookmark);
        if (takenBookmarks.has(refName))
            continue;
        cond.baseline = 'initial';
        fixCount++;
        console.error(`[aiHelper.autoFix] ✓ Orphan bookmark "${refName}" on conditionBeat ${beat.id} ` +
            `→ converted to baseline:'initial'. ` +
            `(No upstream bookmarkAffectState Effect with that name was found.)`);
    }
    if (fixCount > 0) {
        console.error(`[aiHelper.autoFix] Auto-converted ${fixCount} orphan bookmark reference(s) to baseline:'initial'`);
    }
}
/**
...
async function generateStoryWithAI(config, apiKey) {
    const { prompt, genre = 'adventure', length = 'medium', complexity = 'moderate', context, affectDepth = 'auto' } = config;
    ...
    // v0.9.46+ — affect-aware section gates how much of the rich-character /
    // affect system the AI deploys. SYNC SOURCE: packages/core/src/prompts/
    // affectPrompt.ts. The MCP servers don't depend on @asaps/core so this
...
Generate a complete, engaging interactive story structure.`;
    ...
    // Auto-fix: orphan bookmark references (v0.9.46+). The AI tends to author
    // baseline:{bookmark:"X"} refs without the upstream bookmarkAffectState
    // Effect. Convert orphans to baseline:'initial' so the condition reads
    // sanely instead of comparing against zero.
    autoFixOrphanBookmarkReferences(generated);
