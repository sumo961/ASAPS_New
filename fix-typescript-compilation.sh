...
echo ""
echo "1. Fixing AssetSelectionModal.tsx..."

# Create a simpler AssetSelectionModal that doesn't pass unsupported props
...
export const AssetSelectionModal: React.FC<AssetSelectionModalProps> = ({
  ...
  // Filter assets based on type, subtype, and search
      ...
      <div className="bg-white rounded-lg shadow-xl w-[900px] h-[700px] flex flex-col">
        ...
        <div className="p-4 border-b">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder={`Search ${assetSubType || assetType || 'assets'}...`}
              value={searchTerm}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {filteredAssets.length === 0 ? (
            <div className="text-center py-12 text-gray-500">
              <p>No {assetSubType || assetType || 'assets'} available</p>
        ...
        <div className="p-3 border-t bg-gray-50 text-xs text-gray-600">
          ...
          {assetSubType && ` • Category: ${assetSubType}`}
        </div>
      </div>
    </div>
  );
...
echo "✅ Fixed AssetSelectionModal.tsx"

echo ""
echo "2. Removing incomplete GlobalSettingsInspector-enhanced.tsx..."
rm -f packages/builder/src/components/settings/GlobalSettingsInspector-enhanced.tsx
echo "✅ Removed incomplete file"

echo ""
echo "3. Checking for any incorrect imports..."
# Check if App.tsx has the wrong import
if grep -q "GlobalSettingsInspector-enhanced" packages/builder/src/App.tsx 2>/dev/null; then
  echo "Found incorrect import in App.tsx, fixing..."
  sed -i.bak "s/GlobalSettingsInspector-enhanced/GlobalSettingsInspector/g" packages/builder/src/App.tsx
  echo "✅ Fixed import in App.tsx"
fi

echo ""
echo "4. Running TypeScript build to verify fixes..."
cd packages/builder
npm run build
...
if [ $? -eq 0 ]; then
  ...
  echo "- Fixed AssetSelectionModal to work with existing AssetManager"
  echo "- Removed incomplete GlobalSettingsInspector-enhanced.tsx"
  echo "- Fixed any incorrect imports"
  echo ""
  echo "The build is now working correctly!"
else
  echo ""
  echo "===================================="
