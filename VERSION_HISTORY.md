...
| Feature | Status |
|---------|--------|
| **Counter binding — meters that read affect state** | **v0.9.89**: a counter can be a *display* rather than a mechanic. A new optional `source` turns one into a read-only window onto a character's **sentiment**, **emotion level** or **mood**; the value is projected on every draw and can't be assigned (write surfaces show derived counters disabled with the affect effect that *does* move them, while conditions keep reading them freely). *One projection rule, no setting*: the bar originates at zero wherever zero falls in `[min, max]` — `min: 0` keeps the familiar left-filling gauge, `min: -100` grows outward from the centre so distrust reads as distrust rather than as "less trust"; `min` therefore declares whether the quantity has an opposite. Range defaults never hide state (signed stores default bipolar). *Qualitative bands*: author-set thresholds render a phrase (*strong distrust · wary · neutral · trusting · deep trust*) via `numericFormat: 'band'`, with ladders suggested per source kind — bipolar ones carry a neutral band because sentiments start at zero for everyone. *Authoring*: source picker phrased about the fiction ("you set it at moments" vs "it responds to what happens"), an interactive projection preview (no runtime value exists at authoring time, so a "live" readout would be fiction), and a `min: 0` hint when a sentiment is named after a palette emotion. *Opening stances*: `suggestOpeningStance` proposes a starting sentiment from Agreeableness (NEO-PI-R facet A1 *is* Trust), capped at ±0.35 — suggested only, and silent for characters with no personality so the affect opt-out survives. *HUD*: meter frames now name the character they belong to (two stacked frames were indistinguishable, and only one moved). *Ships with*: starter template "Counters that read affect", character template "Character with affect meters", and a worked example. Two latent defects fixed: exported stories rendered every placed meter at 0 (the player never set a counter resolver — only the Preview Window did), and float noise (`-43.99999999999999`) reached players verbatim. Character templates also now disclose the personality they carry |
| **Security: Electron 43, vite 8, URL scheme hardening** | **v0.9.88**: security-only release, open dependency alerts **39 → 9**. *Electron 40 → 43.3.0* — 40 reached end of life, so GHSA-9f4c-93c8-jc8g (sandboxed iframe bypasses `allow-popups`, high) had no backport; it matters here because `webviewTag` is enabled for the WebView beat, exactly the scenario the advisory covers. *URL scheme validation* — both `setWindowOpenHandler`s and the `shell:open-external` IPC handler passed untrusted URLs straight to `shell.openExternal`, which lets the OS launch a protocol handler for `file:`/`smb:`/custom schemes; now http/https only. *vite 5 → 8.2.1* across 9 workspaces (+ plugin-react 6, vite-plugin-dts 5, vite-plugin-electron 1.1), clearing 27 vite + 1 esbuild advisories that had no 5.x backport; two real breakages fixed — Rolldown's stricter CJS handling exposed a missing `react/jsx-runtime` external in the library builds, and stale nested vite@5 lock entries npm refused to prune. Remaining 9 = `@capacitor/cli` tar chain (mobile build CLI) + unreachable `uuid` |
| **Ink & Brass theme, unified HUD layout, HUD explanations** | **v0.9.87**: *New default theme "Ink & Brass"* — literary dark ground (`#14161f`), Georgia titles, brass pill buttons; every visible beat and HUD swept to fit across device sizes and both layout modes (last hardcoded blues gone, speaker labels honour `nameColor`, HUDs render in slot mode). *Unified screen-HUD layout*: `layoutScreenHuds` is the single packer for timer / countdown / mood / meter / inventory — groups by corner, orders by kind, stacks so nothing overlaps **across types**; global HUDs are reserved obstacles; the Character Editor shows an author-facing schematic driven by the same function. *Distraction-free title screens*: screen HUDs hidden on `titleScreen` by default, `hudOverlays.showOnTitleScreen` opts back in. *HUD explanation system*: callouts over the real packed HUD positions, as a standalone `explanation` beat or an `explainHuds` overlay on any beat; beat held `inert` until acknowledged so the player can't click past it; captions default per HUD kind and are author-overridable and translatable. Character counters seed the runtime store and resolve scoped-first in meter frames |
