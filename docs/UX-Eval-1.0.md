...
| # | Proposal | Resolves |
|---|---|---|
| B1 | **"Show only if…" per-choice conditions — REVISED 2026-08-21 (Hartmut design review): guards on edges.** The ASAPS design rule, now stated explicitly: **logic is visible where it acts** — flow branches are beats (conditionCheck), option gates are guards ON THE EDGE they gate (dashed edge + ◇ badge + condition summary in the label; hover = full list, click = editor), and nested structure (dialogTree) EXPANDS instead of hiding. Authoring stays in the Inspector exactly as choices themselves do — authored in the panel, visible in the graph. This is the statechart convention (guards on transitions), and it beats the duplicate-beat pattern on visibility too: the condition sits on the branch it gates, not one hop upstream behind 2^N beat variants that must be hand-synced forever. conditionCheck keeps its distinct job (branching flow vs gating options) and the duplicate-beat pattern remains available. Reachability analysis must treat conditional choices as conditional edges (part of B1, not a follow-up). **Phasing: B1a** = guards on edges + condition editor for top-level choice beats + inventory/variable/counter templates + AI-gen guidance (same-change rule); **B1b** = dialogTree disclosure-triangle expansion — collapsed node as today (phase dots), expanded = container rendering the internal tree read-only with the same guard badges on internal edges, outgoing connections visibly originating from internal exit nodes, click-through to the node editor; aiDialogTree same for its authored skeleton; **B1c** (optional) = in-place editing inside the expansion. | The #1 authoring gap |
