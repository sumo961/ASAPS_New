...
**Earned options — choices that appear only when they've been earned.** Directly under a choice's **Target** in the Inspector sits a small **Show only if…** editor (the same one Movement Choice destinations, Pick Prop props, and hotspots carry). Add a condition there and the choice is *hidden* until every condition holds — an inventory **item** the player has (or lacks), a **variable** value, a **counter** comparison, a **visited beat** (visited or not yet visited), or a **feeling** — how a character feels toward the player (or another character): pick who feels it, the emotion (*trust*, *fear*, any name from your emotion palette; leave it empty to sum all feelings), whom it is toward, and a strength between −1 and 1 (0.3 is mild, 0.6 strong). Tick *since start* to test how much the feeling has *changed* rather than where it stands. This is the tool for the "Ask about the mysterious symbol" option that should only exist once the player has actually found the note. Two habits make it pay off:
...
**Renaming a character.** Change a character's name or display name in the Character Manager and every speaker *linked* to that character follows: beat speakers, Dialog Tree node speakers, and the character's TTS voice assignment all switch to the new name in one step, and a single **Undo** reverts all of it. Places that merely *spell* the old name as free text are not changed behind your back — renaming *"Guard"* to *"Marco"* shouldn't turn every other unlinked guard into Marco. Instead ASAPS lists them and asks: **Link and rename** turns them into linked references with the new name, **Keep "Guard"** leaves them as they are. Stories generated by AI usually store speakers as free text, so the first rename in such a story is where this question comes up.
...
### What We Measured: Which Model Writes the Best Story?

In September 2026 we ran a blind bake-off: the same two briefs (a short one and a
510-word Ideator-length one) generated by each Claude model with identical
settings, the resulting stories injected as playable projects and ranked
blind by two judges. The verdict was unanimous across ten stories:

| Model | Result | Time per story | Cost relative to Opus 5 |
|---|---|---|---|
| **`claude-opus-5`** (adaptive) | **Won both rounds.** Best prose, most ambitious structure (act breaks, fictional clocks, affect-driven scenes) | 6–12 min | 1× |
| `claude-opus-4-8` (no thinking) | Fastest by far, largest beat counts — but the flattest prose, and both its stories had dead logic gates and no clusters | 2–4 min | ~0.5× |
| `claude-fable-5-1` at **X-High** | The cleanest interaction *engineering* (conditional choices, sentiment-split endings, consequences for bluffing) at smaller scope | 13–14 min | ~3× |
| `claude-fable-5-1` at **Max** | Finished (Fable 5 at Max did not) and produced the best-designed accusation scene in the corpus — at the highest cost | ~19 min | ~4× |

What to take from it:

- **Opus 5 is the default for story generation.** Use Opus 4.8 for fast rough
  drafts you intend to rework; use Fable 5.1 when structural correctness
  matters more than scope or speed.
- **Fable always thinks.** There is no reasoning-off setting on Fable models —
  the effort dropdown only sets how *much*. On an Ideator-length brief Fable
  5.1 spends 45–68K tokens thinking before it writes a word, so ASAPS gives
  Fable models a **96K token floor** regardless of the effort you pick. If you
  set **Max Tokens** manually, do not go below that for Fable, or the story
  JSON will be cut off mid-way.
- **The best authoring model is not the best in-story model.** Players wait
  live on every AI Dialog Tree turn; measured on dialog trees, `claude-opus-4-8`
  was both fastest and best there. Set it as the *Model for in-story AI* and
  keep Opus 5 for authoring.
...
**Earned option** - A choice, destination, or prop with a *Show only if…* condition, so it appears only once the story state allows it (a visited beat, an item held, a variable or counter value, or a character's feeling toward someone). The alternative to duplicating a whole menu with and without the option. See [Earned options](#conditional-choices).
